...
inline std::int8_t get_hex_digit(char c) {
    if (c >= '0'&&c <= '9')
        return c - '0';
    if (c >= 'A'&&c <= 'F')
        return c - 'A' + 10;
...
std::string normalize_IPv6(const char *s, const char *e) {
    ...
    for (size_t idx = 0; idx < ntokens; ++idx) {
        const char *pinner = tokens[idx];
        if (pinner == tokens[idx + 1] || *pinner == ':')
            null_pos = idx;
        else {
            std::uint16_t field = get_hex_digit(*pinner++);
            while (p != tokens[idx + 1] && *pinner != ':')
                field = (field << 4) | get_hex_digit(*pinner++);
